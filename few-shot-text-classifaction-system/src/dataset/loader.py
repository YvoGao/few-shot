...
# 这里有fewrel和rcv1的实验没有做，暂时不需要
def _get_cnews_classes(args):
    val_classes = list(range(0, 3))
    train_classes = list(range(3, 6))
    test_classes = list(range(6, 10))
...
def load_dataset(args):
    ...
    elif args.dataset == 'cnews':
        train_classes, val_classes, test_classes = _get_cnews_classes(args)
    else:
        raise ValueError(
            'args.dataset should be one of'
            '[20newsgroup, amazon, fewrel, huffpost, reuters_domyself, rcv1]')
    # 断言表达式，如果表达式为真，可以往下进行
